...
Для машины разработчика есть **lite-режим**: 30 инструментов (по умолчанию публикуются 21 —
...
Альтернатива для разовой привязки — stdio per-session (`claude mcp add … onec-lite`); на больших
конфигурациях FTS может не успеть собраться за сессию. Порт — `ONEC_LITE_PORT` (дефолт 8010).
...
Поэтому профиль по умолчанию (`lean`, 21 инструмент) не публикует `search_code`,
