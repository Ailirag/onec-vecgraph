...
Для машины разработчика есть **lite-режим**: 30 инструментов по живой рабочей копии
...
**Развёртывание — по умолчанию предпочтителен HTTP-вариант.** Для постоянного использования
поднимайте lite как долгоживущий HTTP-сервис с админкой (`uv run onec-lite admin` →
`http://127.0.0.1:8010/mcp`): переживает сессии, обслуживает несколько проектов по заголовку
запроса, а FTS-индекс успевает собраться и держится тёплым в фоне. Вариант stdio (регистрация
через `claude mcp add … onec-lite`) — для разовой per-session привязки; на больших
конфигурациях индекс может не успеть собраться за сессию. **Агент, устанавливающий этот MCP,
обязан явно предложить пользователю выбор способа развёртывания (HTTP-сервис vs stdio
per-session), а не выбирать молча** — см. [docs/LITE_USAGE.md](docs/LITE_USAGE.md).
