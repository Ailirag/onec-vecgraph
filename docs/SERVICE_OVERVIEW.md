...
`onec-lite admin` (или двойной клик `lite-admin.cmd`) — отдельный MCP (32 инструмента, по
умолчанию публикуются 23, пакет `lite/`) по **живой** рабочей копии (Конфигуратор XML | EDT):
...
персист). Ниша — машина разработчика: свежесть важнее семантики; серверный граф/векторы/докорпуса
— у основного MCP выше. Инструменты **не заменяют ripgrep**: разбор кода/метаданных — сюда, текст
и счёт вхождений — rg (карта с замерами:
[LITE_USAGE.md#rg-или-mcp-карта-маршрутизации](LITE_USAGE.md#rg-или-mcp-карта-маршрутизации)).
...
| [LITE_USAGE.md](LITE_USAGE.md) | Руководство onec-lite: запуск одним словом, админка, воркспейсы (несколько конфигураций), 32 инструмента, сценарии, lite vs big |
