...
```
Опционально — лёгкий **read-only веб-дашборд** статуса джоб на том же порту: `ADMIN_DASHBOARD_ENABLED=true`
→ `http://127.0.0.1:8002/jobs` (HTML с автообновлением) и `/jobs.json`. **Без аутентификации** — только
loopback / за прокси. По умолчанию выключен.
