...
По скорости на ПЕРВОМ вопросе сессии тут в основном МЕДЛЕННЕЕ grep: платится прогрев рабочей
копии (~2 с на 15 тыс. модулей). Сразу быстрее только вопросы по индексу имён (объявления x2.2,
места вызова x1.2-1.3). Но прогрев платится ОДИН раз: со второго вопроса ответы идут за
0.002-0.1 с против 0.1-1.9 с у grep. Отсюда правило: один разведочный вопрос дешевле и быстрее
закрыть grep'ом, серию вопросов по одной области — здесь. Числа: docs/bench_tools_vs_grep.md.
...
* «сколько всего и по каким объектам» -> bsl_sql: один GROUP BY по индексу вместо выдачи
  строками. Замерено на УТ: распределение вызовов метода по объектам — 462 токена против
  37 437 у find_callers на том же вопросе, полный счёт одним запросом — 163 токена. Это
  дешевле и шелл-конвейера `rg -c | sort | uniq -c`, и не требует выходить из MCP. Оговорка:
  bsl_sql отвечает по ИНДЕКСУ, без живого подмеса — для свежего кода бери find_callers.
...
агрегаты и «сколько чего» -> bsl_sql (SELECT по индексу; пустой sql отдаёт схему);
...
# --------------------------------------------------------------------------- #
# Профили публикации инструментов
# --------------------------------------------------------------------------- #
# Схемы инструментов уходят клиенту в системном промпте на КАЖДЫЙ запрос: все 30 стоят 7210
# токенов. Замеры показали, что часть из них дублирует то, что у агента и так есть, причём
# дешевле: list_routines против `rg '^(Процедура|Функция)' <модуль>` — x0.28 по токенам,
# changed_objects против `git diff --name-status` — x0.20, find_routine против одной строки rg —
# x0.40, а search_code/read_file/read_module — тонкие обёртки над поиском и чтением файла.
# Поэтому по умолчанию публикуется профиль `lean`: инструменты, отвечающие на вопросы, которые
# поиском по тексту выразить НЕЛЬЗЯ (разбор BSL, слияние базы с расширениями, полные счётчики).
# Удалять их нельзя: там, где у агента нет шелла и чтения файлов, они единственный путь —
# поэтому это профиль, а не вырезание. ONEC_LITE_PROFILE=full возвращает все 30.
...
def _profile() -> str:
    return _env("ONEC_LITE_PROFILE", "lean").lower()


def _disabled_tools() -> set[str]:
    raw = os.environ.get("ONEC_LITE_DISABLED_TOOLS", "")
    return {t.strip() for t in raw.replace(",", " ").split() if t.strip()}


def _published(name: str) -> bool:
    """Публикуется ли инструмент в этой конфигурации (профиль + точечное отключение)."""
    ...
    if profile == "full":
        return True
    if profile == "review":
        return name in _REVIEW_PROFILE
    return name not in _REDUNDANT_WITH_SHELL  # lean (по умолчанию)


def _tool(fn):
    """Зарегистрировать функцию как MCP-инструмент, если она публикуется в этом профиле.

    Функция остаётся импортируемой и вызываемой в любом случае — CLI, админка и тесты зовут её
...
@_tool
def bsl_sql(sql: str = "", max_rows: int = 200, workspace: str = "") -> dict:
    """АГРЕГАТЫ по индексу рутин и вызовов: произвольный SELECT (только чтение).

    Пустой sql возвращает СХЕМУ (таблицы, колонки, счётчики строк) и готовые примеры — начни с
    него, схему не угадывай. Таблицы: symbols (рутины), calls (каждое вхождение вызова,
    caller_id -> symbols.id), files, meta.

    Зачем: «сколько и по каким объектам» обычные инструменты отдают строками — на горячем имени
    это десятки тысяч токенов, а один GROUP BY укладывается в сотни. Пример: распределение
    вызовов метода по объектам —
    ...
    Имена в *_low хранятся в нижнем регистре, приводи литерал сам (SQLite не понижает кириллицу).

    Ответ идёт по ИНДЕКСУ: незакоммиченные правки и подтянутое git'ом после сборки здесь НЕ
    учтены — это единственный тул без живого подмеса, поэтому рядом отдаётся блок index. Для
    точного ответа по свежему коду бери find_callers/find_routine."""
    return fts.sql_query(_ws(workspace), sql, max_rows=max_rows)
