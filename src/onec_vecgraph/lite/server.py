...
По скорости на ПЕРВОМ вопросе — паритет (структурные вопросы быстрее в 1.2-3 раза, состав
объекта — на порядки, а git-вопросы (`review_set`, группировка изменений по объектам
метаданных) в 4-5 раз МЕДЛЕННЕЕ: они
делают кратно больше работы). Со второго вопроса по той же области инструменты отвечают из
прогретого индекса за 0.002-0.06 с против 0.1-1.5 с у grep — серию вопросов выгоднее задавать
здесь. Числа и методика: docs/bench_tools_vs_grep.md.
