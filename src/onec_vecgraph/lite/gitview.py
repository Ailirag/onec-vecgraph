...
def review_set(ws: Workspace, ref: str = "", max_callers: int = 5, source: str = "",
    ...
    for row in routines:
        ...
        # Отсекаем ТОЛЬКО доказуемо чужое — квалификатор, равный имени общего модуля. Правило
        # «у форменной цели любой чужой квалификатор доказывает другую цель» было НЕВЕРНЫМ:
        # экспортный метод модуля формы штатно вызывается снаружи по ссылке на форму
        # (`&НаКлиентеНаСервереБезКонтекста Процедура X(Форма)` → `Форма.МетодЭкспорт()`), и
        # такое правило молча обнуляло вызывающих — на УТ 242 реальных места вызова, причём
        # find_callers на тот же вопрос отвечал верно. Квалификатор-переменную опровергнуть
        # нечем, поэтому такие строки остаются.
        kept = [c for c in found
                if not (c.get("qualifier")
                        and c["qualifier"].lower() not in (own, *_SELF_QUALIFIERS)
                        and c["qualifier"].lower() in common_names)]
